...
\begin{proof}
    \leanok
    Pick $x$ and $z$ arbitrarily and let $\alpha (x)$ be the formula 
...
\begin{proof}
    \leanok
    Pick $x$ and $y$ arbitrarily and let $\alpha (x)$ be the formula 
...
\begin{proof}
    \leanok
    Pick $x$ arbitrarily and let $\alpha (x)$ be the formula 
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        Now, note that the formula $\psi(v,u): u = v \lhd y$ satisfies the condition for 
        Theorem \ref{thm:repl_scheme}, i.e. $v \mapsto v \lhd y$ is a well-defined mapping.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item By Theorem \ref{thm:IsOrd.comparability}, we have to exclude $k \in \text{succ}(l)$, 
        i.e. we have to show that neither $k\in l$ nor $k=l$ can occur. 
        This is indeed so because each of these possibilities, together with $l\in k$, 
        leads to $l \in l$.
        \item Suppose, for a contradiction, that $k \neq l$. 
        Then $\text{succ}(k) = k \lhd k = l \lhd l = \text{succ}(l)$ 
...
\begin{proof}
    ...
    Let $\alpha(x)$ be the formula 
    $$(\forall (k \in x)[\text{Ord} k]) \rightarrow (x \neq \varnothing) \rightarrow 
...
\begin{corollary}
    ...
    $\forall k ((\text{Ord}(k) \land k \neq \varnothing) \rightarrow \exists!l(\text{succ}(l)=k))$.
\end{corollary}
...
\begin{proof}
    ...
    either $\text{succ}(\max (k)) = k$ or $\text{succ}(\max (k)) \in k$. 
    However, the latter contradicts the definition of $\max (k)$. 
    Thus $\text{succ}(\max (k)) = k$. Uniqueness follows from 
...
\begin{definition}
    ...
    The unique (if existing) $l$ for which $\text{succ}(l) = k$ is called the 
    \textit{predecessor} of $k$, abbreviated by $\text{pred}(k)$.
\end{definition}
